# Gaussian (Continuous) Likelihood for JAGS/NIMBLE Model Compiler
#
# Generates the observation likelihood block for gaussian-distributed responses,
# handling structured errors (phylogenetic, spatial), random intercepts, and
# the independent (standard GLM) case.
#
# Primary function: jags_likelihood_gaussian()
#
# @keywords internal

jags_likelihood_gaussian <- function(ctx, response, suffix, loop_bound) {
  if (grepl("^p_", response)) {
    ...
    if (isTRUE(aux_flags$skip_likelihood)) return(invisible(ctx))
  }
  ...
  if (ctx$independent) {
      ...
      if (is_unified) {
        ...
        if (isTRUE(s_def$partition_handled)) {
          ctx$param_map[[length(ctx$param_map) + 1]] <- list(
            response = response, predictor = s_name,
            parameter = paste0("lambda_", response), type = "structure"
          )
        }
      } else {
    ...
    ctx$model_lines <- c(
      ctx$model_lines,
      paste0("  for (i in 1:", loop_bound, ") {"),
      paste0("    ", response_var, "[i] ~ dnorm(", mu, "[i]", additive_terms, ", ", tau_res, ")")
    )
    if (ctx$engine == "jags") {
      ctx$model_lines <- c(
        ctx$model_lines,
        paste0("    log_lik_", response, suffix, "[i] <- logdensity.norm(", response_var, "[i], ", mu, "[i]", additive_terms, ", ", tau_res, ")")
      )
    }
    ctx$model_lines <- safe_add_lines(ctx$model_lines, "  }", ctx$declared_nodes)
  }
