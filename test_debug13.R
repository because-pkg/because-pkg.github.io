...
fit <- because(equations = sem8_eq, data = rhino.dat, structure = rhino.tree, id_col = "SP", engine = "numpyro", n.iter=100)

print(colnames(fit$samples[[1]])[!grepl("^z_", colnames(fit$samples[[1]]))])
