...
Abundance_lambda <- exp(0.5 * obs_U_Res + 0.3 * d_obs$Foraging_Intensity - 0.2 * d_obs$Wind_Speed - 0.4 * (d_obs$Temperature - d_obs$Thermal_Tol) + d_obs$survey_total_re + rnorm(nrow(d_obs), 0, 0.1))
...
eqs <- list(
    ...
    Foraging_Intensity ~ U_Resource + Metabolic_Rate + (1 | Site) + (1 | Survey),
    Abundance ~ U_Resource + Foraging_Intensity + Wind_Speed + I(Temperature - Thermal_Tol) + (1|Site) + (1|Survey)
)
suppressMessages(require(because))
model_code <- because_model(eqs, latent="U_Resource", hierarchical_info=list(data=data_list, levels=list(species = c("Body_Mass_s", "Metabolic_Rate", "Thermal_Tol", "Species"), site = c("Elevation_s", "Temperature", "U_Resource", "Site"), survey = c("Wind_Speed", "Survey"), obs = c("Foraging_Intensity", "Abundance")), hierarchy="site > survey > obs; species > obs", link_vars=list(site="Site", survey="Survey", species="Species")), random_structure_names=c("Site", "Survey"), random_terms=list(list(response="Foraging_Intensity", group="Site"), list(response="Foraging_Intensity", group="Survey"), list(response="Abundance", group="Site"), list(response="Abundance", group="Survey")))$model
