...
if (file.exists(RESULTS_FILE)) {
    ...
    if (nrow(existing) > 0) {
        ...
        log_msg("Resuming from iteration", start_iter)
        # Convert existing rows back to a list of data frames
        all_results <- split(existing, seq(nrow(existing)))
    }
}

for (i in start_iter:N_ITERATIONS) {
