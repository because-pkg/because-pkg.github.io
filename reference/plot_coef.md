...
Generic for plotting path coefficients from a fitted model object. For
`because` model objects, see `plot_coef.because`, which produces a
caterpillar plot of raw or marginal-effects estimates with customisable
colour schemes and significance highlighting.
