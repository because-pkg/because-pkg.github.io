...
Let's fit a standard model first. By default, `because` assigns wide Gaussian priors (`dnorm(0, 1.0E-6)`) to intercepts (`alpha`) and slopes (`beta`). For Gaussian response variables, the **residual standard deviation** gets a half-uniform prior: `sigma_e ~ dunif(0, 100)`, from which the precision is derived as `tau_e <- 1 / (sigma_e * sigma_e)`. This is scale-invariant and works safely whether data are standardised or on their original (raw) scale.
