...
Standard R formulas (`y ~ A * B`) handle interactions automatically on complete data. However, if values of A or B are missing, and unless you use the statistically problematic approach of eliminating rows with missing data (with a consequent loss of power and potential bias if missingness is not completely at random), **imputation** introduces a trap. If you follow the common workflow of "impute first, then model", the imputation step usually assumes simple linear relationships and ignores the interaction. This often biases the interaction estimate towards zero.
